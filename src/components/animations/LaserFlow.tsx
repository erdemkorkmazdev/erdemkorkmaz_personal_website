...
const LaserFlow = ({
  ...
  dpr = 1,
  mouseSmoothTime = 0.1,
  mouseTiltStrength = 10.5,
  horizontalBeamOffset = 0.0,
  verticalBeamOffset = 0.0,
  flowSpeed = 0.8,
  verticalSizing = 2.0,
  horizontalSizing = 6.0,
  fogIntensity = 2.8,
  fogScale = 2.0,
  wispSpeed = 1.0,
  wispIntensity = 3.0,
  ...
  useEffect(() => {
    ...
    const material = new THREE.RawShaderMaterial({
      ...
      depthTest: false,
      depthWrite: false,
      blending: THREE.AdditiveBlending // Usually better for lasers/glow
    });
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      mouseTarget.set(x * pr, (rect.height - y) * pr); // Y inverted for GL
    };
    ...
    const animate = () => {
      ...
      // Fade in
      if (uniforms.uFade.value < 1.0) {
        uniforms.uFade.value += dt * 0.5;
      }

      // Smooth mouse
      mouse.lerp(mouseTarget, dt * 10.0);
      uniforms.iMouse.value.set(mouse.x, mouse.y, 0, 0);
  ...
  }, [color, wispDensity, horizontalBeamOffset, verticalBeamOffset, flowSpeed]);
